...
function EncoderBuffer(value, reporter) {
  if (Array.isArray(value)) {
    ...
    this.value = value;
    this.length = Buffer$B.byteLength(value);
  } else if (Buffer$B.isBuffer(value)) {
    this.value = value;
...
EncoderBuffer.prototype.join = function join(out, offset) {
  ...
  if (Array.isArray(this.value)) {
    ...
    else if (Buffer$B.isBuffer(this.value))
...
PEMDecoder.prototype.decode = function decode(data, options) {
  ...
  var input = new Buffer$C(base64, 'base64');
...
DERNode$1.prototype._encodeComposite = function encodeComposite(tag,
  ...
  var header = new Buffer$D(1 + 1 + lenOctets);
  header[0] = encodedTag;
...
DERNode$1.prototype._encodeStr = function encodeStr(str, tag) {
  if (tag === 'bitstr') {
    ...
    var buf = new Buffer$D(str.length * 2);
...
DERNode$1.prototype._encodeInt = function encodeInt(num, values) {
  ...
  return this._createEncoderBuffer(new Buffer$D(out));
};
...
function ecSign (hash, priv) {
  ...
  return Buffer$G.from(out.toDER())
}
...
function getKey (x, q, hash, algo) {
  x = Buffer$G.from(x.toArray());
  if (x.length < q.byteLength()) {
    var zeros = Buffer$G.alloc(q.byteLength() - x.length);
    x = Buffer$G.concat([zeros, x]);
  }
  var hlen = hash.length;
  var hbits = bits2octets(hash, q);
  var v = Buffer$G.alloc(hlen);
  v.fill(1);
  var k = Buffer$G.alloc(hlen);
  k = browser$3(algo, k).update(v).update(Buffer$G.from([0])).update(x).update(hbits).digest();
  v = browser$3(algo, k).update(v).digest();
  k = browser$3(algo, k).update(v).update(Buffer$G.from([1])).update(x).update(hbits).digest();
  v = browser$3(algo, k).update(v).digest();
  return { k: k, v: v }
}
...
function bits2octets (bits, q) {
  ...
  bits = bits.mod(q);
  var out = Buffer$G.from(bits.toArray());
  if (out.length < q.byteLength()) {
    var zeros = Buffer$G.alloc(q.byteLength() - out.length);
    out = Buffer$G.concat([zeros, out]);
  }
  return out
}
...
function makeKey (q, kv, algo) {
  ...
  do {
    ...
    while (t.length * 8 < q.bitLength()) {
      ...
      t = Buffer$G.concat([t, kv.v]);
    }

    k = bits2int(t, q);
    kv.k = browser$3(algo, kv.k).update(kv.v).update(Buffer$G.from([0])).digest();
    kv.v = browser$3(algo, kv.k).update(kv.v).digest();
  } while (k.cmp(q) !== -1)
...
Object.keys(algorithms$2).forEach(function (key) {
  algorithms$2[key].id = Buffer$I.from(algorithms$2[key].id, 'hex');
...
Sign.prototype.update = function update (data, enc) {
  if (typeof data === 'string') data = Buffer$I.from(data, enc);
...
Verify.prototype.verify = function verifyMethod (key, sig, enc) {
  if (typeof sig === 'string') sig = Buffer$I.from(sig, enc);
...
var mgf = function (seed, len) {
  var t = Buffer$J.alloc(0);
  var i = 0;
  var c;
  while (t.length < len) {
    c = i2ops(i++);
    t = Buffer$J.concat([t, browser$2('sha1').update(seed).update(c).digest()]);
  }
  return t.slice(0, len)
};

function i2ops (c) {
  var out = Buffer$J.allocUnsafe(4);
...
function withPublic (paddedMsg, key) {
  return Buffer$K.from(paddedMsg
...
function oaep (key, msg) {
  ...
  var mLen = msg.length;
  var iHash = browser$2('sha1').update(Buffer$L.alloc(0)).digest();
  var hLen = iHash.length;
  var hLen2 = 2 * hLen;
  if (mLen > k - hLen2 - 2) {
    throw new Error('message too long')
  }
  var ps = Buffer$L.alloc(k - mLen - hLen2 - 2);
  var dblen = k - hLen - 1;
  var seed = browser(hLen);
  var maskedDb = xor(Buffer$L.concat([iHash, ps, Buffer$L.alloc(1, 1), msg], dblen), mgf(seed, dblen));
  var maskedSeed = xor(seed, mgf(maskedDb, hLen));
  return new bn(Buffer$L.concat([Buffer$L.alloc(1), maskedSeed, maskedDb], k))
}
function pkcs1 (key, msg, reverse) {
  ...
  var ps;
  if (reverse) {
    ps = Buffer$L.alloc(k - mLen - 3, 0xff);
  } else {
    ps = nonZero(k - mLen - 3);
  }
  return new bn(Buffer$L.concat([Buffer$L.from([0, reverse ? 1 : 2]), ps, Buffer$L.alloc(1), msg], k))
}
...
var privateDecrypt = function privateDecrypt (privateKey, enc, reverse) {
  ...
  var zBuffer = Buffer$M.alloc(k - msg.length);
  msg = Buffer$M.concat([zBuffer, msg], k);
  if (padding === 4) {
